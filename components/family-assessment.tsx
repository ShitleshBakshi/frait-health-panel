...
export function FamilyAssessment({ familyId, assessmentId, mode = "edit"}: FamilyAssessmentProps) {
    ...
    const handleFinalize = async () => {
        ...
        try {
            ...
            const status = user?.role === "POW_EFRAIT_AssistantHealthVisitors" ? "PENDING_APPROVAL" : "DONE";
            ...
            toast({
                ...
                description: "The assessment has been successfully finalised.",
            });
                        ...
                        <div className="flex items-center justify-between">
                            <AlertDescription>
                                {isAssessmentComplete
                                    ? "Your assessment is ready to be " + (user?.role === "POW_EFRAIT_AssistantHealthVisitors" ? "submitted for approval." : "finalized.")
                            ...
                            {isAssessmentComplete && (
                                <Button
                                    ...
                                    size="sm"
                                    className={user?.role === "POW_EFRAIT_AssistantHealthVisitors"
                                        ? "bg-orange-500 hover:bg-orange-600 text-white"
                                        : "bg-green-500 hover:bg-green-600 text-white"}
                                >
                                    {user?.role === "POW_EFRAIT_AssistantHealthVisitors" ? "Send for Approval" : "Finalise"}
                                </Button>
                            )}
                        </div>
                    </Alert>

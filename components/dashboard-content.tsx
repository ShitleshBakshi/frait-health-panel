...
export function DashboardContent() {
    ...
    if (authUser?.role === "POW_EFRAIT_Admins") {
        return (
    ...
    useEffect(() => {
        if (authUser?.role === "POW_EFRAIT_Managers") {
            const fetchUserCounts = async () => {
                ...
                try {
                    ...
                    const healthVisitorQuery = `
                      query {
                        filterUsers(role: "POW_EFRAIT_HealthVisitors") {
                          id
                        }
                      }
                    `
                    ...
                    const assistantHealthVisitorQuery = `
                      query {
                        filterUsers(role: "POW_EFRAIT_AssistantHealthVisitors") {
                          id
                        }
                      }
                    `
    ...
    if (authUser?.role === "POW_EFRAIT_HealthVisitors") {
        return (
            <div className="p-6 space-y-6 max-w-7xl mx-auto">
                <div>
                    <h1 className="text-2xl font-semibold text-gray-900">
